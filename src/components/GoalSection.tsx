  ...
  <section className="bg-background py-20">
    <div className="container max-w-4xl">
      <h2 className="font-heading text-3xl md:text-4xl font-bold text-primary text-center mb-12 flex items-center justify-center gap-3">
        <Target className="text-primary" size={32} /> {t("Our Goals")}
      </h2>
      <div className="grid md:grid-cols-3 gap-6">
        {goals.map((g) => (
          <div key={g.title} className="text-center space-y-3">
            ...
            <h3 className="font-heading font-bold text-foreground">{t(g.title)}</h3>
            <p className="text-sm text-muted-foreground">{t(g.desc)}</p>
          </div>
        ))}
      </div>
    </div>
